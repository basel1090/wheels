...
class ConstantsController extends Controller
{
        ...
        if ($request->isMethod('GET')) {
            ...
            return DataTables::eloquent($constants)
                ->addColumn('action', function ($constant) {
                    $editBtn = '<a href="' . route('settings.constants.edit', ['constant' => $constant->field]) . '" class="btn btn-icon btn-active-light-primary w-30px h-30px btnUpdateConstant">
    ...
    public function edit(Request $request, $constant)
    {
        // $id = $role = null;
        $request->validate([
            'constant' => 'string'
        ...
        $constants = Constant::where('field', $constant)->get();
